...
pub fn build(b: *std.Build) void {
    ...
    // ============================================================================
    // test-layers - Layer System Testing Tool (Phase 1 verification)
    // ============================================================================
    const test_layers = b.addExecutable(.{
        .name = "test-layers",
        .root_module = b.createModule(.{
            .root_source_file = b.path("src/test_layers.zig"),
            .target = target,
            .optimize = optimize,
        }),
    });

    b.installArtifact(test_layers);

    // Run command for test-layers
    const run_test_layers = b.addRunArtifact(test_layers);
    run_test_layers.step.dependOn(b.getInstallStep());

    const test_layers_step = b.step("test-layers", "Test the layer system (Phase 1)");
    test_layers_step.dependOn(&run_test_layers.step);

    // ============================================================================
    // test-phase2 - Display Layer Integration Testing Tool (Phase 2 verification)
    // ============================================================================
    const test_phase2 = b.addExecutable(.{
        .name = "test-phase2",
        .root_module = b.createModule(.{
    ...
    test_phase2.linkLibC();
    test_phase2.linkLibCpp();

    // Ghostty components
    test_phase2.addIncludePath(b.path("vendor/ghostty/src"));

    // Hermes include paths
    test_phase2.addIncludePath(b.path("src"));
    test_phase2.addIncludePath(b.path("vendor/hermes/API"));
    test_phase2.addIncludePath(b.path("vendor/hermes/API/jsi"));
    test_phase2.addIncludePath(b.path("vendor/hermes/public"));

    // Add uucode module
    test_phase2.root_module.addImport("uucode", uucode_module);

    // Add animation module
    test_phase2.root_module.addImport("animation", animation_module);

    // Add unicode tables
    unicode_tables.addStepDependencies(&test_phase2.step);
    test_phase2.root_module.addAnonymousImport("unicode_tables", .{
        .root_source_file = unicode_tables,
    });

    // Add ghostty_grapheme module
    test_phase2.root_module.addImport("ghostty_grapheme", ghostty_grapheme_mod);

    // Add C++ source files
    test_phase2.addCSourceFile(.{
        .file = b.path("src/jsi/hermes_c_api.cpp"),
        .flags = &[_][]const u8{
            "-std=c++17",
            "-fno-sanitize=all",
    ...
    test_phase2.addLibraryPath(b.path("vendor/hermes/build/API/hermes"));
    test_phase2.addLibraryPath(b.path("vendor/hermes/build/jsi"));
    test_phase2.linkSystemLibrary("hermes_lean");
    test_phase2.linkSystemLibrary("jsi");

    test_phase2.addIncludePath(b.path("vendor/libuv/include"));
    test_phase2.addLibraryPath(b.path("vendor/libuv/build"));
    test_phase2.linkSystemLibrary("uv");

    b.installArtifact(test_phase2);

    // Run command for test-phase2
    const run_test_phase2 = b.addRunArtifact(test_phase2);
    run_test_phase2.step.dependOn(b.getInstallStep());

    const test_phase2_step = b.step("test-phase2", "Test Display layer integration (Phase 2)");
    test_phase2_step.dependOn(&run_test_phase2.step);
