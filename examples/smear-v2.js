...
function getTopBlockProperties(micro_shift, thickness, shade) {
  ...
  // Use inverted mode for top blocks (unless legacy support)
  const character_list = BOTTOM_BLOCKS;  // Flipped for inverted mode
  const hl_group = getHlGroup(hl_group_index, true);  // inverted=true
...
function getRightBlockProperties(micro_shift, thickness, shade) {
  ...
  // Use inverted mode for right blocks (unless legacy support)
  const character_list = LEFT_BLOCKS;  // Flipped for inverted mode
  const hl_group = getHlGroup(hl_group_index, true);  // inverted=true
...
function drawHorizontallyShiftedSubBlock(row, col_left, col_right, shade) {
  ...
  // Alternating bulge logic
  if (Math.max(gap_left, gap_right) / 2 < Math.min(gap_left, gap_right)) {
    if (bulge_above) {
    ...
    for (let col = G.left; col <= G.right; col++) {
      ...
      if (intersections[DIAGONAL] === undefined || intersections[DIAGONAL] < 1 - config.max_shade_no_matrix) {
        ...
        if (is_horizontally_shifted) {
          // Skip very thin right blocks at target position
