...
vim.e2e.describe('Extmarks API', function() {
  ...
  vim.e2e.test('bufGetExtmarks with details option', function() {
    ...
    if (found && found.length === 4) {
      const details = found[3] as { endLine?: number; endCol?: number; hlGroup?: string };
  ...
  vim.e2e.test('bufGetExtmarkById with details returns extended info', function() {
    ...
    if (pos) {
      ...
      if (pos.length === 3) {
        const details = pos[2] as { endLine?: number; endCol?: number; hlGroup?: string };
  ...
  vim.e2e.test('bufSetExtmark preserves priority in details', function() {
    ...
    if (found && found.length === 4) {
      const details = found[3] as { priority?: number };
  ...
  vim.e2e.test('bufSetExtmark with negative endLine/endCol ignores them', function() {
    ...
    vim.e2e.assert.true(found !== undefined, 'should find extmark');
    if (found && found.length === 4) {
      const details = found[3] as { endLine?: number; endCol?: number };
  ...
  // Edge case: bufAddHighlight with negative ns_id returns 0
  vim.e2e.test('bufAddHighlight with negative ns_id returns 0', function() {
    const buf = vim.api.getCurrentBuf();
    const id = vim.api.bufAddHighlight(buf, -1, 'Error', 0, 0, 5);
    vim.e2e.assert.equal(id, 0, 'negative ns_id should return 0');
  });

  // Edge case: bufClearNamespace with negative ns_id does nothing (no error)
  vim.e2e.test('bufClearNamespace with negative ns_id does nothing', function() {
    const ns = vim.api.createNamespace('extmark-clear-neg-ns');
    const buf = vim.api.getCurrentBuf();

    // Create an extmark first
    const id = vim.api.bufSetExtmark(buf, ns, 0, 0, {});

    // Clear with negative ns_id - should do nothing
    vim.api.bufClearNamespace(buf, -1, 0, -1);

    // Extmark should still exist
    const pos = vim.api.bufGetExtmarkById(buf, ns, id, {});
    vim.e2e.assert.true(pos !== null, 'extmark should still exist after clear with negative ns_id');
  });

  // Edge case: bufSetExtmark with negative ns_id returns 0
  vim.e2e.test('bufSetExtmark with negative ns_id returns 0', function() {
    const buf = vim.api.getCurrentBuf();
    const id = vim.api.bufSetExtmark(buf, -1, 0, 0, {});
    vim.e2e.assert.equal(id, 0, 'negative ns_id should return 0');
  });

  // Edge case: bufSetExtmark with negative buffer handle returns 0
  vim.e2e.test('bufSetExtmark with negative buffer returns 0', function() {
    const ns = vim.api.createNamespace('extmark-neg-buf');
    const id = vim.api.bufSetExtmark(-1, ns, 0, 0, {});
    vim.e2e.assert.equal(id, 0, 'negative buffer handle should return 0');
  });
});
