...
vim.e2e.describe("Fetch with AbortController", function() {
  ...
  vim.e2e.test("abort listener calls native __abortFetch", function() {
    ...
    if (typeof __abortFetch !== 'undefined') {
      ...
      (globalThis as any).__abortFetch_original = __abortFetch;
    }
    ...
    // Restore original
    if ((globalThis as any).__abortFetch_original) {
      (globalThis as any).__abortFetch = (globalThis as any).__abortFetch_original;
    }
  });
