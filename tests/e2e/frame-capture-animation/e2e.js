...
vim.e2e.describe("Smear-Cursor Animation Simulation", function () {
    vim.e2e.test("rapid cursor movement produces animation-ready frames", function () {
        ...
        for (let i = 0; i < frames.length; i++) {
            vim.e2e.assert.true(frames[i].output.length > 0, `Animation frame ${i} should have output`);
        }
    ...
    vim.e2e.test("diagonal movement (hjkl combo) captures intermediate positions", function () {
        ...
        vim.e2e.assert.true(frames[0].output !== frames[1].output, "First two diagonal frames should differ");
        vim.e2e.assert.true(frames[1].output !== frames[2].output, "Last two diagonal frames should differ");
    });
...
vim.e2e.describe("Animation Frame Analysis Helpers", function () {
    vim.e2e.test("can detect cursor position codes in frames", function () {
        ...
        vim.e2e.assert.true(matches.length > 0, "Frame should contain cursor position escape codes");
    ...
    vim.e2e.test("can measure frame-to-frame changes", function () {
        ...
        vim.e2e.assert.true(differences > 0, "Frames should have detectable differences");
    ...
    vim.e2e.test("frame output size is consistent for similar renders", function () {
        ...
        vim.e2e.assert.true(sizes.every(s => s > 0), "All frames should have some output");
    });
});
vim.e2e.describe("Animation Performance Characteristics", function () {
    vim.e2e.test("can capture at 60fps simulation rate", function () {
        ...
        vim.e2e.assert.true(totalDuration >= expectedDuration * 0.8, // Allow 20% variance
    ...
    vim.e2e.test("frame capture has low overhead", function () {
        ...
        vim.e2e.assert.true(overhead < 3.0, // Allow up to 3x overhead (capture + memory allocation)
