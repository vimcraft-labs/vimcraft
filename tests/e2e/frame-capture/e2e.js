...
vim.e2e.describe("Animation Frame Capture", function () {
    ...
    vim.e2e.test("multiple frames have increasing timestamps", function () {
        ...
        vim.e2e.assert.true(frames[1].timestamp >= frames[0].timestamp, "Second frame timestamp should be >= first");
        vim.e2e.assert.true(frames[2].timestamp >= frames[1].timestamp, "Third frame timestamp should be >= second");
        vim.e2e.pty.stopFrameCapture();
    });
