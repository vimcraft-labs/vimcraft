...
vim.e2e.describe("process.spawnPty", function () {
    ...
    vim.e2e.test("cwd option changes working directory", function () {
        ...
        vim.e2e.wait(200);
        vim.e2e.assert.true(output.includes("/tmp") || output.includes("/private/tmp"), "cwd should be /tmp, got: " + JSON.stringify(output));
    });
    ...
    vim.e2e.test("env option passes environment variables", function () {
        ...
        vim.e2e.wait(200);
        vim.e2e.assert.true(output.includes("pty_env_test"), "should receive custom env var, got: " + JSON.stringify(output));
    });
    ...
    vim.e2e.test("programs see real TTY (isatty returns true)", function () {
        ...
        vim.e2e.wait(200);
        vim.e2e.assert.true(output.includes("/dev/") || output.includes("tty"), "should be a real TTY device, got: " + JSON.stringify(output));
        vim.e2e.assert.true(!output.includes("not a tty"), "should NOT report 'not a tty'");
    });
    vim.e2e.test("TERM environment variable is set", function () {
        ...
        vim.e2e.assert.true(output.includes("xterm") || output.includes("256color"), "TERM should be set, got: " + JSON.stringify(output));
    });
    vim.e2e.test("resize updates SIGWINCH to process", function () {
        ...
        pty.kill();
        vim.e2e.wait(200);
        vim.e2e.assert.true(output.includes("30") && output.includes("100"), "should report new size, got: " + JSON.stringify(output));
    });
    // =========================================================================
    ...
    vim.e2e.test("bidirectional I/O with interactive shell", function () {
        ...
        vim.e2e.wait(200);
        vim.e2e.assert.true(output.includes("interactive_test"), "should receive echoed command output, got: " + JSON.stringify(output));
        pty.kill();
        vim.e2e.wait(200);
    });
    ...
    vim.e2e.test("multiple PTYs can run concurrently", function () {
        ...
        pty1.onData((data) => { output1 += data; });
        pty2.onData((data) => { output2 += data; });
        pty1.onExit(() => { exits++; });
        pty2.onExit(() => { exits++; });
        vim.e2e.wait(300);
    ...
    vim.e2e.test("three concurrent PTYs with different commands", function () {
        ...
        pty1.onData((d) => { outputs[0] += d; });
        pty2.onData((d) => { outputs[1] += d; });
        pty3.onData((d) => { outputs[2] += d; });
