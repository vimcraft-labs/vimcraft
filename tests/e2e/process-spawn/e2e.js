...
vim.e2e.describe("Process Spawn - Stderr Handling", function () {
    vim.e2e.test("capture stderr from ls nonexistent", function () {
        ...
        vim.e2e.assert.true(result.stderr.includes("No such file") || result.stderr.includes("cannot access"), "stderr should mention file not found");
    });
...
vim.e2e.describe("Process Spawn - Arguments", function () {
    ...
    vim.e2e.test("argument with spaces", function () {
        ...
        vim.e2e.assert.equal(result.code, 0);
        vim.e2e.assert.true(result.stdout.trim() === "hello world with spaces", "Should preserve spaces in single argument");
    });
...
vim.e2e.describe("Process Spawn - Working Directory", function () {
    vim.e2e.test("cwd option changes directory", function () {
        ...
        vim.e2e.assert.equal(result.code, 0);
        vim.e2e.assert.true(result.stdout.trim() === "/tmp" || result.stdout.trim() === "/private/tmp", "Should be in /tmp directory");
    });
...
vim.e2e.describe("Process Spawn - Error Cases", function () {
    ...
    vim.e2e.test("command not found via sh", function () {
        ...
        vim.e2e.assert.true(result.stderr.includes("not found") || result.stderr.includes("command not found"), "Should report command not found");
    });
    vim.e2e.test("permission denied", function () {
        ...
        if (result.code !== 0) {
            vim.e2e.assert.true(result.stderr.includes("Permission denied") || result.stderr.includes("Operation not permitted"), "Should report permission error");
        }
    });
...
vim.e2e.describe("Process Spawn - Environment Info", function () {
    ...
    vim.e2e.test("process.platform is valid", function () {
        ...
        vim.e2e.assert.true(["darwin", "linux", "windows"].includes(platform), "platform should be darwin, linux, or windows, got: " + platform);
    });
    vim.e2e.test("process.arch is valid", function () {
        const arch = process.arch;
        vim.e2e.assert.true(["arm64", "x64", "x86"].includes(arch), "arch should be arm64, x64, or x86, got: " + arch);
    });
    vim.e2e.test("process.env.HOME exists", function () {
        vim.e2e.assert.true(process.env.HOME !== undefined, "HOME should be defined");
        vim.e2e.assert.true(process.env.HOME.startsWith("/"), "HOME should be absolute path");
    });
    vim.e2e.test("process.env.PATH exists", function () {
        vim.e2e.assert.true(process.env.PATH !== undefined, "PATH should be defined");
        vim.e2e.assert.true(process.env.PATH.includes("/"), "PATH should contain paths");
    });
    vim.e2e.test("process.env.USER exists", function () {
        vim.e2e.assert.true(process.env.USER !== undefined, "USER should be defined");
        vim.e2e.assert.true(process.env.USER.length > 0, "USER should not be empty");
    });
    vim.e2e.test("process.env.SHELL exists", function () {
        // SHELL might not exist in all environments
        if (process.env.SHELL) {
            vim.e2e.assert.true(process.env.SHELL.startsWith("/"), "SHELL should be absolute path");
        }
    });
...
vim.e2e.describe("Process Spawn - Real World Commands", function () {
    ...
    vim.e2e.test("git status in repo", function () {
        ...
        vim.e2e.assert.true(result.code === 0 || result.stderr.includes("not a git repository"), "Should either succeed or report not a git repo");
    });
    ...
    vim.e2e.test("ls directory listing", function () {
        ...
        vim.e2e.assert.true(result.stdout.includes("r") && (result.stdout.includes("w") || result.stdout.includes("-")), "Should show permissions");
    });
...
vim.e2e.describe("Process Spawn - Security", function () {
    vim.e2e.test("allowed commands work", function () {
        ...
        for (const cmd of cmds) {
            ...
            vim.e2e.assert.true(result.stderr === undefined || !result.stderr.includes("not allowed"), cmd + " should be allowed");
        }
    });
