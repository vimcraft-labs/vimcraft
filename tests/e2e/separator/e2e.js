...
vim.e2e.describe("Separator Height", function () {
    vim.e2e.test("separator count matches window height", function () {
        ...
        vim.e2e.assert.true(separatorCount >= winHeight - 2, "Separator count (" + separatorCount + ") should be >= window height - 2 (" + (winHeight - 2) + ")");
    });
...
vim.e2e.describe("Separator Continuity", function () {
    vim.e2e.test("separator has no gaps in column", function () {
        ...
        console.log("  Window height: " + winHeight);
        console.log("  Separator count: " + separatorCount);
        vim.e2e.assert.true(separatorCount >= expectedMinSeparators - 1, "Separator count (" + separatorCount + ") should be >= " + (expectedMinSeparators - 1) + " (window height - 1)");
    });
...
vim.e2e.describe("Separator State Consistency", function () {
    vim.e2e.test("separator count stable after 10 cursor movements", function () {
        ...
        vim.e2e.assert.equal(initialCount, afterMovementCount, "Separator count should remain stable (was " + initialCount + ", now " + afterMovementCount + ")");
    });
    vim.e2e.test("separator count stable after 20 rapid movements", function () {
        ...
        vim.e2e.assert.equal(initialCount, finalCount, "Separator should persist through rapid cursor movements");
    });
    vim.e2e.test("separator stable after window switch cycle", function () {
        ...
        vim.e2e.assert.equal(initialCount, finalCount, "Separator should persist through window switching");
    });
    vim.e2e.test("separator stable through insert mode cycle", function () {
        ...
        vim.e2e.assert.equal(initialCount, finalCount, "Separator should persist through insert mode cycle");
    });
    vim.e2e.test("separator exists at statusline row after re-render", function () {
        ...
        vim.e2e.assert.true(separatorCount >= winHeight, "Separator should span at least window height (" + winHeight + "), got " + separatorCount);
    });
    vim.e2e.test("separator stable through 5 render cycles", function () {
        ...
        for (let i = 1; i < counts.length; i++) {
            vim.e2e.assert.equal(firstCount, counts[i], "Frame " + (i + 1) + " separator count should match frame 1");
        }
    });
});
// ============================================================================
...
vim.e2e.describe("Separator Visual Column (Terminal-Accurate)", function () {
    ...
    function getDisplayWidth(codepoint) {
        ...
        if (codepoint >= 0x20000 && codepoint <= 0x2A6DF)
            return 2; // CJK Extension B
        if (codepoint >= 0x3040 && codepoint <= 0x30FF)
            return 2; // Hiragana/Katakana
    ...
    function parseSeparatorColumns(output) {
    ...
    vim.e2e.test("vsplit separator visual column with README.md", function () {
        ...
        vim.e2e.assert.true(separatorPositions.size > 0, "Should find separators");
        vim.e2e.assert.equal(uniqueCols.length, 1, "Separator should be at same visual column on ALL rows. Found: " + uniqueCols.join(", "));
    });
    vim.e2e.test("vsplit with explicit emoji content", function () {
        ...
        vim.e2e.assert.equal(uniqueCols.length, 1, "Separator should be at same visual column with emoji. Found: " + uniqueCols.join(", "));
    });
});
// Run all tests
vim.e2e.runAll();
